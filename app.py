...
def display_kickoff_time(match_data):
    ...
    try:
        unix_timestamp = match_data.get('kickoff')
        if not unix_timestamp:
            st.warning("Kickoff time not available")
            return

        # Convert unix timestamp to datetime in German timezone using timezone-aware objects
        german_tz = pytz.timezone('Europe/Berlin')
        utc_dt = datetime.fromtimestamp(unix_timestamp, pytz.UTC)  # Using UTC timezone
